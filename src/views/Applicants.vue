<template>
  <div>
    ...
    <el-table-column prop="name" label="Họ tên" width="240"> </el-table-column>
    ...
    >
      <template slot-scope="scope">
        <span v-if="scope.row.proof_record.verified" style="color:green;">Đã xác minh</span>
        <span v-if="!scope.row.proof_record.verified" style="color:red;">Chưa xác minh</span>
      </template>
    </el-table-column>
    <el-table-column fixed="right" label="Operations" width="120">
      <template slot-scope="scope">
        <el-button @click="handleClick(scope.row.proof_record)" type="text" size="small"
...
export default {
  ...
  methods: {
    handleClick(proof) {
      ...
      for (let object in this.currentProof.presentation_request.requested_attributes) {
        const name = this.currentProof.presentation_request.requested_attributes[object]
          .name;
